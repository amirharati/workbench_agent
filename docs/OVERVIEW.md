...
- Category management: Taxonomy and dashboard Inspector use one additive, intent-first manager for accepting/rejecting suggestions, attaching multiple existing categories, choosing a primary, and creating/renaming/deleting user taxonomy. One idea searches existing parents/leaves before proposing editable final structure. Mutations are atomic in the DB worker; item-scoped creation verifies the accepted attachment and global creation is explicitly labeled Taxonomy-only.
- Category classification: V3 combines the existing independent purpose-aware LLM result with a second local exact-embedding result over category definitions, member aggregates, and bounded manually accepted examples. A deterministic additive resolver preserves both signals and item-local rejection. When no taxonomy parent fits, the matcher returns a non-category `NO_MATCH`; the DB durably retains its editable novel-topic evidence, Discover consumes it, and category management groups it for Review/Reclassify without polluting taxonomy, search, or counts. The DB worker builds only a small disposable example shard and introduces no new vector library, database, or execution owner. Live gates and the explicit post-V3 ANN/calibration upgrade are defined in [`CATEGORY_CLASSIFICATION_PLAN.md`](CATEGORY_CLASSIFICATION_PLAN.md).
- Inspector presentation: Categories sit directly below the bookmark identity/actions, ahead of AI enrichment, and accepted category chips open the existing all-library category results view. The complete enrichment result—summary, tags, key points, and references—is contained in a bounded, keyboard-focusable scroll region so long content cannot consume the entire right panel; similar links and item metadata remain reachable in the outer Inspector scroll.
...
- Durable runtime generations: before Start/Resume/Recovery, the service worker verifies that the retained offscreen coordinator matches the currently built offscreen entry. An idle stale owner is replaced before accepting work; an active stage is never interrupted merely for replacement. If a hashed lazy module disappears during a run, only that claimed stage is atomically returned to Pending, the coordinator is recreated, and recovery continues from already committed fetch/AI/embed checkpoints. Ordinary provider/model errors remain normal task failures, and repeated module-load failures stop after bounded retries.
