...
- Category classification plan: V3 will combine the existing independent purpose-aware LLM result with a second local exact-embedding result over category definitions, member aggregates, and bounded accepted examples. A deterministic additive resolver preserves both signals and item-local rejection; no new vector library or execution owner is introduced. The staged implementation and explicit post-V3 ANN/calibration upgrade are defined in [`CATEGORY_CLASSIFICATION_PLAN.md`](CATEGORY_CLASSIFICATION_PLAN.md).
