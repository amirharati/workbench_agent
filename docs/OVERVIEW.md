...
- Category classification: V3 combines the existing independent purpose-aware LLM result with a second local exact-embedding result over category definitions, member aggregates, and bounded manually accepted examples. A deterministic additive resolver preserves both signals and item-local rejection; the DB worker builds only a small disposable example shard and introduces no new vector library, database, or execution owner. Live gates and the explicit post-V3 ANN/calibration upgrade are defined in [`CATEGORY_CLASSIFICATION_PLAN.md`](CATEGORY_CLASSIFICATION_PLAN.md).
