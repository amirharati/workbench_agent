...
- Category management: Taxonomy and dashboard Inspector use one additive manager for accepting/rejecting suggestions, attaching multiple existing categories, choosing a primary, and creating parent/child categories. Mutations are atomic in the DB worker; exact duplicates are blocked and optional semantic profile matching suggests similar categories and likely parents before creation.
