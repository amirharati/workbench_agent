...
| **Search / RAG** | **Hybrid is the default**: parsed exact rules stay deterministic, while query embeddings rank matches and surface separately labelled related material against worker-owned document vectors. Topical categories have persisted worker-owned semantic profiles that blend taxonomy text with reliable member-vector centroids; pipeline quality/attention labels are explicitly excluded. Search suggests categories and tags as exact Search-local tabs, preserves the originating query for back-and-forth review, and uses query-to-category confidence to rank linked Related results. Retrieval/storage remain local; query/category-text embedding uses optional key-backed AI and visibly falls back to text. |
