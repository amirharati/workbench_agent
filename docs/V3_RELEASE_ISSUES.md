...
| V3-020 | 2026-08-22 | Search / categories | Category names affected exact matching, category semantics were not durably indexed, category/tag exploration replaced the current activity, and pipeline-quality leaves leaked into topic discovery | P2 | READY TO RETEST | Persisted worker-owned metadata + member-centroid profiles for topical leaves only; quality/attention leaves excluded from every Search category path; confidence-weighted expansion; exact category/tag results open in closeable Search-local tabs initialized with visible, composable `category:` / `tag:` syntax; discovery is above results when narrow and beside results when wide | Current category-search-profile checkpoint | Reload for schema/protocol v9/v21; search finance/trading and confirm Login/Redirect quality labels never appear; open several category/tag tabs, switch/close them without losing the original query or leaving Search; refine one with keywords and combine exact fields using AND/`+` and OR; resize across the responsive boundary; repeat a semantic query to verify the persisted fast path |
