...
- [ ] While Chrome's install/New Tab prompt is present, cancel or interrupt the folder picker; confirm the
      blocking Homebase modal remains and explicitly says nothing changed.
- [ ] Select a folder and confirm Homebase lists its recognized files before **Use this folder** persists it.
- [ ] Confirm the completion modal names the folder, reports the item count, and distinguishes files loaded,
      migrated, kept, or created; onboarding must not complete without this receipt.
...
| V3-017 | 2026-08-17 | Organization / drag-and-drop | Item dragging was fragmented, result views and membership containers had ambiguous semantics, and legacy collection drops guessed the source from the first membership | P1 | READY TO RETEST | One versioned item contract for URLs and notes; global Workspace/Collection tray plus visible drop zones; Project All items resolves to one real collection (directly when unique, explicit chooser when multiple); reference sources add only; same-type containers ask Copy/Move; cross-type drops copy; workspace reorder; lossless collection placement transfer and Undo | `8012821` + Project All-items checkpoint | Reload and run the staged drag matrix: Search/Similar/All Library/Enrichment → Workspace and Collection; Project All items with one and multiple collections; Collection ↔ Collection within/across projects; Workspace ↔ Workspace including Global; cross-type copy-only; already-present; Undo; reload persistence |
| V3-018 | 2026-08-17 | Navigation / preview context | Search and Enrichment could erase organization context, item actions could silently create/activate workspace state or redirect to Home, and detail review competed with the Inspector | P1 | READY TO RETEST | Keep activity, organization scope, active workspace, and temporary preview independent; persistent project/collection and active-workspace controls; one app-wide non-navigating Peek with stored content, ordered Previous/Next, inline organization, explicit View workspace, and explicit Open original | 2026-08-17 context/Peek checkpoint | From Search, Similar, Library, Project, Notes, and Enrichment: select, Preview, Previous/Next, organize, close, and confirm the activity/query/filter/scroll/selection/scope/workspace are unchanged; then test explicit active-workspace change and View workspace separately |
| V3-019 | 2026-08-18 | Workspace management | Named workspace creation was hidden behind an ambiguous Save icon; creation implicitly cloned the active set, management was fragmented, and Search only displayed the active workspace | P2 | READY TO RETEST | Shared project manager with explicit empty-by-default New, optional Copy active, Activate/Rename/Merge/Delete, deduplicating same-project merge, responsive contextual switchers limited to Global plus open-project workspaces, and per-project General defaults with remembered intentional overrides | Uncommitted 2026-08-18 workspace-management fix | In a project and on Workspaces: create empty and copied sets, activate/rename/merge/delete, verify General is protected and items survive; switch X/Z/All Library and confirm each defaults/restores correctly; confirm activity/query/results remain mounted and controls do not overlap |
...
| V3-024 | 2026-08-23 | Classification / user evidence | A newly accepted manual category is present in the LLM catalog but does not reliably influence later similar bookmarks; duplicate/mis-parented manual leaves can split evidence or fail hierarchy validation | P1 | READY TO RETEST | Full-taxonomy two-pass LLM plus independent DB-worker exact-embedding classifier over definition, member aggregate, and bounded manually accepted examples; deterministic additive resolver; item-category-local rejection | [`CATEGORY_CLASSIFICATION_PLAN.md`](CATEGORY_CLASSIFICATION_PLAN.md), protocol v28 implementation | Run seed-only, Airbnb positive/control, pair-rejection, one/five/bulk, cancel/navigation/close/sleep/Resume, and 20k latency/memory gates; duplicate-category prevention/merge remains separately visible |
| V3-025 | 2026-08-24 | Organization / group transfer | Lists and Search could copy or move only one saved item at a time, making workspace and collection curation impractical | P1 | READY TO RETEST | Shared canonical multi-item payload; list selection with Select all filtered; Search result-set selection; one destination chooser; same-type Copy/Move, cross-type Copy; selected-group drag; rollback on partial collection-write failure and one group Undo. The adaptive chooser keeps Quick access, Global, and every open project in one searchable catalog; its large-screen project/detail panes become a mouse-only project drill-down on narrow screens. Releasing a drag over a project freezes the payload in this chooser so the mouse need not stay held. | `6d1f613`, `a0311b8`; adaptive/frozen chooser follow-up uncommitted | In All Library, a filtered collection, Search, and Workspaces: select individual rows, Select all filtered/results, drag one selected row, and use Organize selected. Verify direct drop, release-over-project then click, project switching/search, narrow drill-down, same-type Copy/Move, cross-type copy-only, already-present counts, Undo, refresh persistence, hidden filtered rows untouched, and the same complete open-project catalog from every source. |
...
- Runtime-generation follow-up — 2026-08-23: a one-link Airbnb job completed V2 fetch, AI, and embedding,
  then failed Classify because a retained offscreen coordinator requested a hashed lazy chunk removed by a
  newer build. New Start/Resume/Recovery requests now compare the retained owner's actual module URL with
  the current `offscreen.html` entry and replace only an idle stale owner. A mid-run missing-module error
  atomically requeues only the claimed stage, preserves all earlier committed work, recreates the owner, and
  recovers the same job; two failed runtime retries remain bounded and become a real failure. Protocol v27
  exposes the safe requeue RPC and retained-owner generation/active-stage metadata.
