...
| V3-017 | 2026-08-17 | Organization / drag-and-drop | Item dragging was fragmented, result views and membership containers had ambiguous semantics, and legacy collection drops guessed the source from the first membership | P1 | READY TO RETEST | One versioned item contract for URLs and notes; global Workspace/Collection tray plus visible drop zones; reference sources add only; same-type containers ask Copy/Move; cross-type drops copy; workspace reorder; lossless collection placement transfer and Undo | 2026-08-17 universal drag checkpoint | Reload and run the staged drag matrix: Search/Similar/All Library/Enrichment → Workspace and Collection; Collection ↔ Collection within/across projects; Workspace ↔ Workspace including Global; cross-type copy-only; already-present; Undo; reload persistence |
