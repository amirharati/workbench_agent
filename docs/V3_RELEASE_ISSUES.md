...
| V3-024 | 2026-08-23 | Classification / user evidence | A newly accepted manual category is present in the LLM catalog but does not reliably influence later similar bookmarks; duplicate/mis-parented manual leaves can split evidence or fail hierarchy validation | P1 | DESIGN AGREED — IMPLEMENTATION PENDING | Keep the full-taxonomy two-pass LLM classifier and add an independent DB-worker exact-embedding classifier over category definition, member aggregate, and bounded accepted examples; merge outputs deterministically and additively; rejection remains item-category-local | [`CATEGORY_CLASSIFICATION_PLAN.md`](CATEGORY_CLASSIFICATION_PLAN.md) | Implement in five checkpoints, then run seed-only, Airbnb positive/control, pair-rejection, one/five/bulk, cancel/navigation/close/sleep/Resume, and 20k latency/memory gates |
