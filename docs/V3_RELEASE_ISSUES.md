...
| V3-025 | 2026-08-24 | Organization / group transfer | Lists and Search could copy or move only one saved item at a time, making workspace and collection curation impractical | P1 | READY TO RETEST | Shared canonical multi-item payload; list selection with Select all filtered; Search result-set selection; one destination chooser; same-type Copy/Move, cross-type Copy; selected-group drag; rollback on partial collection-write failure and one group Undo | Uncommitted group-organization checkpoint | In All Library, a filtered collection, Search, and Workspaces: select individual rows, Select all filtered/results, drag one selected row, and use Organize selected. Verify Workspace→Workspace and Collection→Collection Copy/Move, Workspace↔Collection copy-only, already-present counts, Undo, refresh persistence, and that hidden filtered rows are untouched. |
