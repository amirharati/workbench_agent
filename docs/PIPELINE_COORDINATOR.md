...
clients: they may submit, pause, cancel, or inspect work, but they never own or execute a pipeline job.
...
- At most one job executes at a time in the first implementation.
- Interactive single-item work is submitted to the same coordinator and may be ordered ahead of bulk work,
  but does not start a second executor.
...
Dashboard tabs / side panel / Import Studio
          |  submit IDs/options, pause, cancel, subscribe
          v
Extension service worker (routing and wake-up only)
          |
          v
One offscreen Pipeline Coordinator
          |-- high-priority interactive lane
          |-- one serialized bulk lane
          |-- bounded per-job network concurrency
          |
          v
Core DB worker
          `-- workbench.sqlite (jobs, tasks, library, derived metadata)
          |
          `-- separate content worker
                    `-- workbench-content.sqlite (fetched bodies)
```
...
- `status` — `queued`, `running`, `cancel_requested`, `cancelling`, `paused`, `completed`, `failed`, or
  `cancelled`
- `payload_json` — versioned execution options and scoped checkpoint
...
The normalized task rows make progress and recovery queryable without loading a 10,000-item JSON document.
The job payload may still contain the existing scoped-wave checkpoint during the first implementation, but
SQLite owns it and task rows remain the progress index.
...
The first coordinator runs one job at a time and one item at a time. After its acceptance tests pass, bounded
per-item concurrency may be introduced behind the same durable stage model. A single never starts another
executor or runs in a dashboard page.
...
1. Expire leases whose deadline passed.
2. Return abandoned `running` tasks to `pending`.
3. Recover the oldest previously running job before ordinary queued bulk work.
4. Resume from committed task/stage state.
5. Reject writes from an older lease generation and broadcast the canonical snapshot to all open clients.
...
Manual **Resume** is a durable state transition from `paused`/`failed` to `queued`; it does not destroy a worker,
clear a cross-tab lock, or reconstruct ownership in a dashboard. Normal interruption recovery is automatic.

## Pause and cancel

- **Pause:** stop launching new work, abort at a supported safe boundary, retain pending tasks, set `paused`.
- **Cancel:** durably record `cancel_requested`, abort every stage, then set remaining tasks/job `cancelled`
  only after the executor has stopped. Completed outputs remain.
...
after a provider accepted a request but before its response committed is recorded as an uncertain attempt;
the system must not silently spend again without the configured retry policy or user approval.
...
- Starting jobs from multiple dashboards still produces one serialized bulk lane.
- A single-link digest submitted during bulk begins in the coordinator's priority lane.
...
- Resume continues only unfinished tasks and never waits on a page-owned lock.
- Cancel and Pause have distinct durable outcomes.
...
- Folder permission/sync delay does not block live processing or Resume.
