...
- [x] **Persisted category search profiles (V3, 2026-08-22):** `ai_category_search_profiles` stores the category metadata vector, a bounded centroid sampled from reliable shared bookmark embeddings, and their adaptive blended prototype. The DB worker refreshes stale membership evidence and Search backfills missing taxonomy-text vectors, ranks query→category without sending stored vectors to tabs, suggests categories as exact Search-local tabs, and confidence-weights linked Related results. Tags likewise open exact local result tabs. The `link-quality` diagnostic tree is excluded from profiles, category matching, Search documents, facets, and Similar/category expansion rather than being treated as topical evidence. Near-duplicate category merge remains separate future work.
