...
- [x] **V3 manual category management (2026-08-23):** Taxonomy and dashboard Inspector share one intent-first manager for additive existing-category assignment, Accept/Reject, remove-from-bookmark, primary selection, and parent/child creation. One category idea searches the full taxonomy, then an optional AI naming step proposes editable final parent/child names and optional descriptions; a new structure atomically creates both its named child and standard fallback. User-created categories support rename and guarded deletion; the DB worker owns every atomic create, attach, rename, branch-delete, and durable rejection. Deleting taxonomy never deletes bookmarks or enrichment. Exact duplicates are blocked; lexical plus cached-profile semantic matching warns about near duplicates and suggests likely parents. Classify can prefill a novel-topic draft but never auto-creates a singleton category.
