...
- [x] **V3 product query semantics (2026-08-08):** Product Search now parses default-AND terms, exact quoted phrases, OR groups, explicit AND/`+`, `-` exclusions, and `site:` guards; displays the interpretation; separates exact primary matches from semantic related results; ranks against full embeddings inside the canonical DB worker without hydrating vectors into dashboard tabs; and reports semantic/text-fallback execution honestly.
