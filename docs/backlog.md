...
- [ ] **Phase B — Fetch enrichment v1 (for categorization quality):** Add a **best-effort, non-blocking** enrichment pipeline using `r.jina.ai` as default for public URLs. If fetch fails, categorization must continue using existing item text (title/url/notes/tags). Include: (1) an **independent fetch service module** (provider interface + adapters) so enrichment/categorization/UI call one stable API and we can add/swap providers later without app-wide rewrites, (2) preflight eligibility pass that outputs **fetchable now** vs **excluded with reason** (`non_http`, blocked/private domain, recently failed, already-fresh), (3) two run modes sharing one pipeline — **smart fetch** (uncategorized/low-confidence subset with cap) and **full/batch fetch** (global recategorize support), (4) per-item fetch state tracking (`status`, attempts, `lastFetchAt`, `lastErrorCode`, `nextRetryAt`) with backoff and retry limits, (5) explicit failure classes (`timeout`, `rate_limited`, `auth_required`, `parse_empty`, `network`) surfaced in UI/reporting, (6) hard request budgets (max items per run, timeout/token caps) and stop/resume hooks, (7) explicit note that authenticated pages are **best-effort only** in V1 (no assumption of shared logged-in session/cookies).
...
- [ ] **Data model (parallel semantic layer):** New IndexedDB stores (or versioned migration in `db.ts`) for **`ai_categories`** (id, name, status `ai_proposed|approved|manual`, centroid vector ref or inline small vector, optional LLM-generated canonical tag list, `created_at`/`updated_at`), **`ai_item_category_links`** (`itemId`, `categoryId`, score, `isPrimary`, optional secondaries with **hard cap** e.g. max 1–2 secondary, `source` manual|ai, `status` suggested|accepted|rejected), **`ai_item_signals`** (per-item tags/keywords/confidence, `textHash`, `embeddingModel` id, `lastProcessedAt`). **Link to `items` by `itemId` only** — do not replace `projects`/`collections` in V1 for this track.
- [ ] **Backup / export:** Extend `exportDB` / `importDB` / `verifyBackup` for new keys when stores land. **Policy:** backup **structured AI fields** (categories, links, signals, hashes, model ids); **raw fetch bodies** stay out of main JSON (disk/cache); **raw embedding vectors optional** in backup (prefer **recompute from `textHash` + stored text** to keep JSON small — document tradeoff).
- [ ] **Pipeline (bottom-up, no LLM in core loop):** Per link: build `itemText` → optional fetch (existing **Fetch enrichment v1** task) → **one doc-level embedding** per item (local small model first, e.g. MiniLM-class via worker) → similarity to **category centroids** → assign primary (+ secondaries if near-tie) OR send to **novelty pool** if below threshold. **No “recluster every N links” on LLM** — batch **novelty pool** on cadence (count threshold, manual button, or schedule) using embedding math; **LLM optional** only for naming a proposed cluster / polishing canonical cluster tag vocabulary.
- [ ] **Tags after category:** Do **not** generate free-form tags before assignment. After assignment: derive per-link tags from **cluster canonical tags + link text** (embedding similarity + light lexical rules), **normalize/dedupe** (`cpp`↔`c++`, caps), **cap** (e.g. 3–5 tags). Optional small LLM pass later — not V1 default.
...
**Active product thread:** Near-term roadmap above (AI infra → **bulk import polish + scale assessment** → **AI — V1** semantic layer + fetch enrichment + doc embeddings → **Ask AI** polish → **AI — V2** hybrid search/chunks as needed → broader RAG/agentic).
...
*Last updated: 2026-05-13 — Added **AI — V1 / V2 / Later** roadmap: parallel **AI categories** + doc embeddings + bottom-up assignment/novelty pool; **fetch service** detail; **two-stage retrieval** + chunks at V2; **optional cloud** embed/search/GPU path with pluggable interfaces; **V2 taxonomy DAG** item retained; suggested order updated.*
