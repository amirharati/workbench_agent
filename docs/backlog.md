...
> Product rename (V3): **Homebase**. Historical bullets may still mention older names.
...
- **Backup UX**: Dashboard **Settings** view (`SettingsView`) — backup status panel, manual backup button (`manual-YYYY-MM-DD_HHMMSS.json`), and conflict resolution actions (Load remote / Keep local overwrite). First setup and folder changes use a two-phase select/inspect/confirm flow; interruption never persists the candidate, and a non-dismissible completion receipt reports the folder and file actions. **Home** is a placeholder until product decides what belongs there.
...
- **Side panel polish**: Themed like dashboard (CSS variables / primitives); bookmark-only flow with notes under URL; owning-tab prefill; “already saved” with edit / remove / add new copy; after save, latest version selected; duplicate bookmark blocked for the same URL in the same collection. The native lifecycle is lazy and contextual: an eligible toolbar gesture creates the existing-window cohort, later tabs remain untouched, dashboard and `chrome://extensions` tabs stay excluded, and native close affects only one tab’s presentation while shared data/pipeline work continues. The action on extension management opens/focuses the dashboard. Install/startup/reload never configure/open/close panels. Set Homebase as Home remains a settings/clipboard helper.
...
- [ ] **Post-V3 — Organization suggestions / constrained librarian agent** — Help place links and notes into manual **projects and especially collections** without silently reorganizing the library. Entry points should include a selected Search result, Item/Inspector, Inbox/Unfiled, post-import review, and an explicit selected-item/batch action. First rank existing destinations using saved content, enrichment, semantic categories, current search/project context, and the user's prior accepted placements; show confidence plus a short reason. Propose a new collection only when several items form a coherent gap, with a preview of its name, purpose, members, and target project. An optional `Auto-suggest` mode may prepare recommendations, but AI work must have an explicit activation/budget boundary and never spend per keystroke. Applying remains review-first: approve/reject individually or in a batch, display the exact membership diff, preserve multi-collection placement, record provenance and rejected suggestions, and support undo. A later agent mode may use tightly scoped `suggest`, `create collection`, and `add placement` tools, but it must preview its plan and receive confirmation before any durable create/move/remove action.
...
- [x] **Bootstrap empty library:** A clean broad seed provides 23 stable topical parents and 90 durable starter leaves, generates one General leaf per parent, and keeps Link-quality outside the topic tree. Discover grows supported children and genuinely missing domains; no k-means-only bootstrap is required. Category search profiles embed parent + leaf definitions in the background when credentials exist, then blend reliable assigned-link vectors as evidence accumulates.
- [x] **V3 manual category management + abstention (2026-08-24):** Taxonomy and dashboard Inspector share one intent-first manager for additive existing-category assignment, Accept/Reject, remove-from-bookmark, primary selection, and parent/child creation. One category idea searches the full taxonomy, then an optional AI naming step proposes editable final parent/child names and optional descriptions; a new structure atomically creates both its named child and standard fallback. User-created categories support rename and guarded deletion; the DB worker owns every atomic create, attach, rename, branch-delete, and durable rejection. Deleting taxonomy never deletes bookmarks or enrichment. Exact duplicates are blocked; lexical plus cached-profile semantic matching warns about near duplicates and suggests likely parents. Classify may explicitly return non-category `NO_MATCH`; its durable per-item novel-topic evidence is grouped in category management, passed to Discover, and can be queued for classify-only retry after taxonomy changes without refetching.
...
- [x] **V1.1 / Task 03 — Pipeline hardening (done 2026-05-26):** Incremental classify-by-hash, quality gate tiers, retry→`manual_review`, run stats, discover CLI loop, **Enrichment dev hub** (Bookmarks: Results / Enrich / Categories), queue reconcilers, LLM no-topic→`pending_discover`, discover pool includes unassigned. The original accept/reject/change-category UX gap is now closed by the V3 shared category manager. Spec + return: [`docs/temp/TASK-03-v1.1-pipeline-hardening.md`](temp/TASK-03-v1.1-pipeline-hardening.md).
...
- [x] **V3 product query semantics (2026-08-08; extended 2026-08-22):** Product Search parses default-AND terms, exact quoted phrases, OR groups, explicit AND/`+`, `-` exclusions, `site:` guards, and exact `tag:` / `category:` membership atoms; displays the interpretation; separates exact primary matches from semantic related results; ranks against full embeddings inside the canonical DB worker without hydrating vectors into dashboard tabs; and reports semantic/text-fallback execution honestly. Clicking a suggested category or tag opens a local tab initialized with the same visible, editable syntax, so membership can be combined with keywords or additional fields.
...
- [x] **Persisted category search profiles + product browse (V3, 2026-08-22):** `ai_category_search_profiles` stores the category metadata vector, a bounded centroid sampled from reliable shared bookmark embeddings, and their adaptive blended prototype. The DB worker refreshes stale membership evidence and Search backfills missing taxonomy-text vectors, ranks query→category without sending stored vectors to tabs, suggests categories as exact Search-local tabs, and confidence-weights linked Related results. Home Categories uses a bounded worker-owned membership snapshot for hierarchical All Library/project browsing and persistent multi-select OR results. Tags likewise open exact local result tabs. The `link-quality` diagnostic tree is excluded from profiles, category matching, product category browsing, Search documents, facets, and Similar/category expansion rather than being treated as topical evidence. Near-duplicate category merge remains separate future work.
