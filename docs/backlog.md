...
- [ ] **Phase B — Fetch enrichment v1 (for categorization quality):** **~95% shipped** — see [`docs/temp/TASK-01-fetch-enrichment-v1.md`](temp/TASK-01-fetch-enrichment-v1.md) (**Resume here** section). Hybrid fetch + AI extract + Review UI live; **close-out = manual extension smoke test**. Original spec: best-effort non-blocking pipeline, provider interface, smart/full batch, per-item state, disk cache. **Follow-ups (add when closing Task 01):** enrichment **AI prompt tuning** (per sourceKind), **deep fetch** depth, **tab provider in extension**, run-level **failure stats / aggregates** (`lastErrorCode`, `aiStatus` buckets), t.co unroll, pick-best provider, AI-only re-run on existing snippet, import enrich hook.
...
*Last updated: 2026-05-20 — Task 01 fetch enrichment ~95% shipped; link to TASK-01 doc + follow-ups (prompt tuning, deep fetch, failure stats).*
